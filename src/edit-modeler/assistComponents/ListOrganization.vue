<template>
     <div>
      <vxe-table
              ...
              <vxe-table-column field="name" title="机构名称" ></vxe-table-column>
       </vxe-table>
      <vxe-pager
          border
          size="medium"
      ...
      methods:{
        getOrganizationData() {
          const that = this;
            ...
            request({
              url: `/workflow/listOrganizations/${that.tablePage.pageSize}/${that.tablePage.pageNo}`,
              method: 'get'
            })
            .then(function(res) {

              if(res.data.status == "1") {
                that.organizationData = res.data.list;
