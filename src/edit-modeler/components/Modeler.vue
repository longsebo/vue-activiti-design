<template>
  <div>
    <el-container style="height: 600px">
      <el-aside width="75%" style="border: 1px solid #DCDFE6" >
...
export default {
  ...
  methods: {
      ...
      saveXML(){
        ...
        that.bpmnModeler.saveXML({format:true},function(err,xml){
            ...
            that.bpmnModeler.saveSVG(function(err, svg) {
              ...
              params.append('bpmn_xml',xml);
              params.append('svg_xml',svg);
              console.log('name:'+that.params.name);
              params.append('name',that.params.name);
              request.post(
                `/workflow/model/${modelId}/xml/save`,
                params,
      ...
      importBpmnXml(){
        ...
        //console.log("load xml:"+that.params.bpmnXml);
        if(that.params.bpmnXml) {
          bpmnXml = that.params.bpmnXml
          that.params.bpmnXml =null;
        }else{
          return;
        }
        //console.log("importBpmnXml xml");

        that.bpmnModeler.importXML(bpmnXml,function(err){
            if(err){
                console.log('bpmn文档导入失败');
                //解决新的模型无法画图问题
                that.bpmnModeler.createDiagram();
                that.success();
            } else {
              // 绑定监听事件
              that.success()
          ...
          eventBus.on(eventType, function(e) {
            if (eventType === 'element.changed') {
              ...
              if (!e || e.element.type == 'bpmn:Process') {
                ...
                that.element = e.element;
                //console.log('id:'+that.key+',propsComponent:'+that.propsComponent+',type:'+e.element.type);
              }
